...
abi:
	...
	SOLC="${ABIGEN_DOCKER}" ./scripts/solc.sh /build/bindings /solidity /solidity/contracts/NFT721.sol

.PHONY: bindings
bindings:
	mkdir -p temp/
	abigen --abi build/bindings/NFT1155.abi --bin build/bindings/NFT1155.bin --pkg nft --out temp/NFT1155.go --type NFT1155
	abigen --abi build/bindings/NFT721.abi --bin build/bindings/NFT721.bin --pkg nft --out temp/NFT721.go --type NFT721